...
namespace GE
{
	...
	void SkyboxRenderer::createCubemap(std::vector<std::string> filenames)
	{
		...
		for (int faceNum = 0; faceNum < 6; faceNum++)
		{
			...
			// Copy the pixel data from the SDL Surface object to the OpenGL texture 
		...
		// when rendering onto an object. GL LINEAR is weighted average of the colours around the texture co-ords 
		glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

		// Clamp the text edge to deal with inaccuracies. If the xyz co-ords go over 
		// under 8/1, then sampler will wrap a texture resulting in artefacting at the edges 
	...
	void SkyboxRenderer::draw(Camera* cam)
	{
		...
		// and store in vertexlocation attribute. Note, there is no arribute pointer for uvs
